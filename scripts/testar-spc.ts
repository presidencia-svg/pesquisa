...
async function main() {
  ...
  const user =
    process.env.SPC_USER ??
    process.env.SPC_API_USER /* nome antigo do Melhores, fallback */
  const senha =
    process.env.SPC_PASSWORD ??
    process.env.SPC_API_PASSWORD /* nome antigo do Melhores, fallback */

  if (!user || !senha) {
    console.error('❌ SPC_USER ou SPC_PASSWORD ausente no .env.local')
    console.error(
      '   Aceita tambem nomes antigos do Melhores: SPC_API_USER / SPC_API_PASSWORD',
    )
    process.exit(1)
  }
  ...
  console.log(
    '  Usuario: ',
    user.slice(0, 3) + '***' + ' (len=' + user.length + ')',
  )
